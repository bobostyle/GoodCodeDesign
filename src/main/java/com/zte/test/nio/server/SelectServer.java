package com.zte.test.nio.server;
...
public class SelectServer {
	...
	public void openServer()throws IOException{
		while(true){
			...
			int keys = selector.select(); //阻塞式的
			if(keys > 0){
				Iterator<SelectionKey> itKeys = selector.selectedKeys().iterator();
				while(itKeys.hasNext()){
					SelectionKey key = itKeys.next();
					if(key.isAcceptable()){
						System.out.println("request from client, accept function is activaction!");
						...
						System.out.println("receive request from client:" + clientChannel.getRemoteAddress());
